"""v58 = v57 recipe with operator encoders refit on all 12 months (MB8).

Base = lgbm_r_all_v58 (uses lgbm_r_all_v58.encoders.pkl at serve time).
R_norm = v55 (12-month refit), gate = v56 (12-month refit).
Extra columns = tempo + p2575 + plan_taxi_res_v57 (12-month medians).
Post-processing MS1 + MS2 as before.
"""
...
def main() -> None:
    ...
    ap.add_argument("--pre-ms", default="kind-mango_v58_pre_ms.parquet")
    ap.add_argument("--out", default="kind-mango_v58.parquet")
    args = ap.parse_args()
    ...
    log.info("=== v58: MB3 base with 12-month encoders + v55 R_norm + v56 gate ===")
    predict_v30_main(
        args.pre_ms,
        r_norm_files=R_NORM_FILES,
        base_model="lgbm_r_all_v58",
        extra_columns=extra,
    ...
    v58, _1 = apply_ms1(pred_vec, adep, flt_null, mvt_eobt1, support)
    v58, _2 = apply_ms2(v58, adep, support)
    assert_ms4(v58, template["MVT_ID_mvt"].values, m["MVT_ID_mvt"].values)
    out = pd.DataFrame({"MVT_ID_mvt": template["MVT_ID_mvt"].values,
                        "TAXITIME_SEC_mvt": v58.astype(np.float64)})
