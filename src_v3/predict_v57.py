...
def main() -> None:
    ...
    # v57 extra_columns: tempo + p2575 + PLAN v57 (all-months medians)
    ...
    log.info("=== v57: MB3 base retrained with v57 medians + v55 R_norm + v56 gate ===")
    predict_v30_main(
        args.pre_ms,
    ...
    v57, _1 = apply_ms1(pred_vec, adep, flt_null, mvt_eobt1, support)
    v57, _2 = apply_ms2(v57, adep, support)
    assert_ms4(v57, template["MVT_ID_mvt"].values, m["MVT_ID_mvt"].values)
    out = pd.DataFrame({"MVT_ID_mvt": template["MVT_ID_mvt"].values,
                        "TAXITIME_SEC_mvt": v57.astype(np.float64)})
