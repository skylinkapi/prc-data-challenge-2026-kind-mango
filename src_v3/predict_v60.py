"""v60 = v57 recipe with MD4 (OOF) operator encoders.

Base = lgbm_r_all_v60. Same served encoders (full-year map at serve
time; training saw leave-one-month-out values). Everything else stays
at v57: v55 R_norm, v56 gate, v57 plan medians, MS1+MS2.
"""
...
def main() -> None:
    ...
    ap.add_argument("--pre-ms", default="kind-mango_v60_pre_ms.parquet")
    ap.add_argument("--out", default="kind-mango_v60.parquet")
    args = ap.parse_args()
    ...
    log.info("=== v60: MD4 OOF encoders base + v55 R_norm + v56 gate ===")
    predict_v30_main(
        args.pre_ms,
        r_norm_files=R_NORM_FILES,
        base_model="lgbm_r_all_v60",
        extra_columns=extra,
    ...
    v60, _1 = apply_ms1(pred_vec, adep, flt_null, mvt_eobt1, support)
    v60, _2 = apply_ms2(v60, adep, support)
    assert_ms4(v60, template["MVT_ID_mvt"].values, m["MVT_ID_mvt"].values)
    out = pd.DataFrame({"MVT_ID_mvt": template["MVT_ID_mvt"].values,
                        "TAXITIME_SEC_mvt": v60.astype(np.float64)})
