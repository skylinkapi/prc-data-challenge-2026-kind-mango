...
| kind-mango_v54 | pending | — | Fifteenth-pass Phase 2 MB1: anchored-offset target `y - anchor` (anchor = mvt_eobt1 if present else sched_delay) on top of MB3. Score boosters directly, add anchor, clip to per-airport offset range. Big rewrite of the target. Non-LIRF mean shifts +3-25 s vs v51; EHAM notably +25 s and 101 rows over 7200. |
