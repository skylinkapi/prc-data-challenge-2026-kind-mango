"""lgbm_v26 = v24 + Step 7 runway configuration categorical."""
...
from features_runway_config import add_runway_config, RWY_CONFIG_CAT_COLS
from train_lgbm_v21 import add_obt_features, CAT_COLS as CAT_COLS_BASE, BEST_PARAMS
...
CAT_COLS = CAT_COLS_BASE + RWY_CONFIG_CAT_COLS
...
def main():
    ...
    ctx_rwy = m[["ADEP_mvt","ADES_mvt","PHASE_mvt","mvt_ts","RUNWAY_mvt"]].copy()
    del m; gc.collect()
    ...
    dep = add_runway_config(dep, ctx_rwy)
    del ctx_rwy; gc.collect()
    ...
    # Cap runway config to top-100 per airport to control cardinality
    top_cfg = train["rwy_config_prev_30m"].value_counts()
    keep_cfg = set(top_cfg.head(200).index)
    train["rwy_config_prev_30m"] = train["rwy_config_prev_30m"].where(
        train["rwy_config_prev_30m"].isin(keep_cfg), other="OTHER")
    test["rwy_config_prev_30m"] = test["rwy_config_prev_30m"].where(
        test["rwy_config_prev_30m"].isin(keep_cfg), other="OTHER")
    print(f"rwy_config distinct in train: {train['rwy_config_prev_30m'].nunique()}")
    ...
    print(f"Feature count: {len(feat)}  (v24 had 127)")
    ...
    print(f"\n=== v26 hold-out RMSE (v24 in parens) ===")
    print(f"FULL   :  {rmse(y_te, p_pos):>7.2f} s   (v24 452.23)")
    clean = (y_te >= 30) & (y_te <= 7200)
    print(f"CLEAN  :  {rmse(y_te[clean], p_pos[clean]):>7.2f} s   (v24 281.92)")
    tail = y_te > 7200
    print(f"TAIL   :  {rmse(y_te[tail], p_pos[tail]):>7.2f} s   n={tail.sum():,}")

    imp = pd.Series(b.feature_importance(importance_type="gain"),
                    index=feat).sort_values(ascending=False)
    r = list(imp.index).index("rwy_config_prev_30m") + 1
    print(f"\nrwy_config_prev_30m rank {r}/{len(feat)}  gain {imp['rwy_config_prev_30m']:.2e}")

    out_dir = os.path.join(ROOT, "models")
    b.save_model(os.path.join(out_dir, "lgbm_v26.txt"))
    with open(os.path.join(out_dir, "lgbm_v26.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v26.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    with open(os.path.join(out_dir, "lgbm_v26.rwy_config_vocab.pkl"), "wb") as f:
        pickle.dump(sorted(keep_cfg), f)
    print(f"\nSaved -> {out_dir}/lgbm_v26.*   total wall {time.time()-t_all:.1f}s")
