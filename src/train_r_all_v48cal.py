"""L8: paired test of calendar columns (holidays + weekend) on v45 features.

Adds is_public_hol, is_weekend, is_hol_or_wknd to v45's 117-column feature
set. Paired vs shipped v45; gate 200 MSE served clean.
"""
import json
import logging
import os
...
CAL = os.path.join(MODELS, "calendar_train.parquet")
EXTRA_COLS = ["is_public_hol", "is_weekend", "is_hol_or_wknd"]
...
def load_frame() -> tuple[pd.DataFrame, list[str]]:
    ...
    dep = dep.merge(pd.read_parquet(CAL), on="MVT_ID_mvt", how="left")
    return dep, feat
...
def train_v48_members(train, stop, test, feat) -> tuple[np.ndarray, list[int]]:
    ...
    for s in DEFAULT_SEEDS:
        ...
        log.info("v48cal seed %d best iter %d in %.0fs", s, b.best_iteration, time.time() - t0)
...
def main() -> None:
    ...
    log.info("scoring shipped v45 members")
    p_v45 = score_v45(test, v45_cols)
    log.info("training v48cal members (v45 recipe + calendar)")
    ...
    report = {"extra_cols": EXTRA_COLS,
              ...
              "v48cal": {"best_iters": iters, **class_table(p_v48, y, cls, apt)}}
    served = {}
    for k in ("clean", "fallback", "tail", "24h"):
        ctl = report["control_v45"]["class_mse"][k] - report["control_v45"]["per_airport"]["LIRF"][k]
        new = report["v48cal"]["class_mse"][k] - report["v48cal"]["per_airport"]["LIRF"][k]
        served[k] = new - ctl
    report["delta_served_v48cal_minus_v45"] = served
    for k in ("control_v45", "v48cal"):
    ...
    log.info("served clean price: %+.0f MSE (gate >= 200)", -served["clean"])
    with open(os.path.join(MODELS, "lgbm_r_all_v48cal.holdout.json"), "w") as f:
