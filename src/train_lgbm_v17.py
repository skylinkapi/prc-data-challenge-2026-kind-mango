"""lgbm_v17 = v15 recipe + OPDI extended event-type rolling counts (12 more)."""
import glob
import os
import pickle
import time
...
from features_opdi_extended import add_opdi_ext, OPDI_EXT_NUM_COLS
...
def main():
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols + op_cols + TAXI_NUM_COLS + ADV_NUM_COLS + OSM_PATH_NUM_COLS + OPDI_NUM_COLS + OPDI_LIVE_NUM_COLS + OPDI_EXT_NUM_COLS
    ...
    print(f"\nLGBM v17 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v15 = 293.09)")
    ...
    print("\nPer-airport RMSE (v17 vs v15):")
    print(tab.sort_values("rmse").to_string())

    print("\nOPDI-extended feature ranks:")
    ...
    b.save_model(os.path.join(out_dir, "lgbm_v17.txt"))
    with open(os.path.join(out_dir, "lgbm_v17.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v17.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v17.*")
