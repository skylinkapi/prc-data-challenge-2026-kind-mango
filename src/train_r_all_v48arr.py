"""L3.e: paired test of arrival taxi-in residual on top of v45 features."""
import json
import logging
import os
import time
...
ARR = os.path.join(MODELS, "arr_txi_res_train.parquet")
EXTRA_COLS = ["arr_txi_res_30", "arr_txi_cnt_30"]
...
def load_frame() -> tuple[pd.DataFrame, list[str]]:
    ...
    dep = dep.merge(pd.read_parquet(ARR), on="MVT_ID_mvt", how="left")
    return dep, feat
...
def train_v48_members(train, stop, test, feat) -> tuple[np.ndarray, list[int]]:
    ...
    for s in DEFAULT_SEEDS:
        ...
        log.info("v48arr seed %d best iter %d in %.0fs", s, b.best_iteration, time.time() - t0)
...
def main() -> None:
    ...
    report = {"extra_cols": EXTRA_COLS,
              ...
              "v48arr": {"best_iters": iters, **class_table(p_v48, y, cls, apt)}}
    served = {}
    for k in ("clean", "fallback", "tail", "24h"):
        ctl = report["control_v45"]["class_mse"][k] - report["control_v45"]["per_airport"]["LIRF"][k]
        new = report["v48arr"]["class_mse"][k] - report["v48arr"]["per_airport"]["LIRF"][k]
        served[k] = new - ctl
    report["delta_served_v48arr_minus_v45"] = served
    for k in ("control_v45", "v48arr"):
    ...
    with open(os.path.join(MODELS, "lgbm_r_all_v48arr.holdout.json"), "w") as f:
