"""lgbm_v15 = v11 recipe + OPDI live-taxi rolling features (mean/median/count)."""
import glob
import os
import pickle
import time
...
def main():
    ...
    print("\nOPDI live-taxi coverage in training:")
    for c in OPDI_LIVE_NUM_COLS[:3]:
        print(f"  {c:40s} coverage {train[c].notna().mean()*100:5.1f}%")
    ...
    print(f"\nLGBM v15 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v11 = 294.18)")
    ...
    print("\nPer-airport RMSE (v15 vs v11):")
    print(tab.sort_values("rmse").to_string())

    print("\nOPDI-live feature ranks (of 124 total):")
    ...
    b.save_model(os.path.join(out_dir, "lgbm_v15.txt"))
    with open(os.path.join(out_dir, "lgbm_v15.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v15.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v15.*")
