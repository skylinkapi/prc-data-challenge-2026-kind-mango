"""Apply v15 (best single model) to ranking.parquet."""
import os
import pickle
import sys
import time
...
def main(out_name: str = "kind-mango_v6.parquet"):
    t0 = time.time()
    ...
    b = lgb.Booster(model_file=os.path.join(MODELS_DIR, "lgbm_v15.txt"))
    with open(os.path.join(MODELS_DIR, "lgbm_v15.features.txt")) as f:
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v6.parquet"
    main(name)
