"""v27 submission = v26 pipeline with:
  - 7-seed R_all_v27 mean (feature_fraction_seed varied)
  - 3 dead features dropped (ice_accretion_1hr, ades_arr_atfm_delay_today, taxi_dist_known)
  - Step 2 fixes:
      1. ITY340 rule uses `(5/6)*(86,400 + 1,150) + (1/6)*sd`
      2. R_norm_LIRF clipped at 4,431 s before mixture
"""
...
from train_r_all_v27_seeds import SEEDS_V27
...
R_NORM_CLIP = 4431.0  # 99.9 pct LIRF genuine taxi (Section 4.1)
NORMAL_MEAN_LIRF = 1150.0  # LIRF genuine mean, ITY340 fix
...
def main(out_name="kind-mango_v27.parquet"):
    t0 = time.time()
    ...
    with open(os.path.join(MODELS, "lgbm_r_all_v27.encoders.pkl"), "rb") as f:
    ...
    print("R_all_v27 7-seed mean predictions...")
    with open(os.path.join(MODELS, "lgbm_r_all_v27.features.txt")) as f:
        feat_all = f.read().splitlines()
    preds = []
    for s in SEEDS_V27:
        b = lgb.Booster(model_file=os.path.join(MODELS, f"lgbm_r_all_v27_s{s}.txt"))
        preds.append(np.clip(b.predict(dep[feat_all]), 0, None))
        del b; gc.collect()
    r_all = np.mean(preds, axis=0)
    del preds; gc.collect()
    print(f"  R_all: min {r_all.min():.0f}  max {r_all.max():.0f}  mean {r_all.mean():.0f}")

    print("LIRF regime head v23 (with Step 2 R_norm clip)...")
    ...
    # STEP 2 FIX: clip R_norm_LIRF at 4,431 (99.9pct of LIRF genuine taxi)
    ...
    # STEP 2 FIX: ITY340 uses `(5/6)*(86,400 + 1,150) + (1/6)*sd`
    ...
    for i in np.where(ity_mask)[0]:
        ...
        print(f"  row sd={sd_r[i]:.0f}  new_pred={p_final[i]:.0f}  (v26 formula would give ~160,000)")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v27.parquet"
    main(name)
