"""v27 = v25 ensemble regressor + v22 mixture (thr=0.99, sd_gate=7200).

For each ranking row:
  taxi_hat = v25 ensemble weighted prediction
  If (p_fb > 0.99 or p_24h > 0.99) AND sched_delay > 7200:
      p = sched_delay
  else:
      p = taxi_hat
"""
...
from features_congestion import add_congestion   # v1, needed by v22 classifiers
...
def main(out_name="kind-mango_v15.parquet"):
    ...
    print("Features (v25 stack + v21 stack)...")
    ...
    dep = add_weather(dep)
    # Add BOTH v1 and v2 congestion, then v1 opdi + v2 opdi
    ...
    for tag, w in weights.items():
        ...
        ens += w * p
        print(f"  v25 {tag} ({w:.4f})")
    taxi_hat = np.clip(ens, 0, None)

    # v22 classifiers (from v21 stack)
    ...
    p_fb = bfb.predict(dep[clf_feat])
    p_24 = b24.predict(dep[clf_feat])
    sd_vals = dep["sched_delay"].values.astype(float)
    swap = ((p_fb > THRESHOLD) | (p_24 > THRESHOLD)) & (sd_vals > SD_GATE) & ~np.isnan(sd_vals)
    print(f"Mixture swaps: {swap.sum():,} / {len(dep):,}   ({swap.mean()*100:.4f}%)")

    p_final = np.where(swap, sd_vals, taxi_hat)
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v15.parquet"
    main(name)
