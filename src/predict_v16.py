"""Apply v16 solo to ranking.parquet. Same feature build as train_lgbm_v16."""
...
def main(out_name="kind-mango_v8.parquet"):
    t0 = time.time()
    ...
    print("Loading training categories + SSL reference...")
    ...
    with open(os.path.join(MODELS, "lgbm_v16.encoders.pkl"), "rb") as f:
    ...
    b = lgb.Booster(model_file=os.path.join(MODELS, "lgbm_v16.txt"))
    with open(os.path.join(MODELS, "lgbm_v16.features.txt")) as f:
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v8.parquet"
    main(name)
