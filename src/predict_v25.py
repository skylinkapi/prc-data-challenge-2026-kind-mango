"""v25 = v24 pipeline + Step 5 (impute missing July 2026 ec_* from 2025 medians).

The ec_* features are 0 % covered in July 2026 (55 % of scoring). Instead of
retraining, impute with 2025 median per airport-hour so the model does not route
July rows to the NaN branch.
"""
import os, pickle, time, gc, glob, json, sys as _sys
import sys
import numpy as np
import pandas as pd
...
def compute_ec_medians():
    """Compute 2025 median for each ec_* col per airport."""
    print("Computing 2025 ec_* medians per airport for imputation...")
    tdir = os.path.join(ROOT, "training")
    ...
    for f in sorted(glob.glob(os.path.join(tdir, "*.parquet"))):
        ...
        t["month"] = t["mvt_ts"].dt.month
        frames.append(t[["ADEP_mvt", "mvt_ts", "sched_ts", "month"]])
    m = pd.concat(frames, ignore_index=True)
    m = add_eurocontrol(m)
    # per airport medians
    med = m.groupby("ADEP_mvt")[ec_cols].median()
    print(f"  medians computed for {len(ec_cols)} ec_* cols over {len(TARGET_ICAOS)} airports")
    return med, ec_cols


def main(out_name="kind-mango_v25.parquet"):
    t0 = time.time()
    ec_medians, ec_cols = compute_ec_medians()
    ...
    with open(os.path.join(MODELS, "lgbm_r_all_v24.encoders.pkl"), "rb") as f:
    ...
    # STEP 5 IMPUTATION: fill NaN ec_* columns per airport median
    print("Imputing missing ec_* with 2025 airport medians...")
    for apt in TARGET_ICAOS:
        apt_mask = dep["ADEP_mvt"] == apt
        if apt not in ec_medians.index: continue
        med_row = ec_medians.loc[apt]
        for c in ec_cols:
            fill_val = float(med_row[c]) if not pd.isna(med_row[c]) else 0.0
            nan_mask = apt_mask & dep[c].isna()
            if nan_mask.sum() > 0:
                dep.loc[nan_mask, c] = fill_val
    n_filled = 0
    for c in ec_cols:
        n_filled += dep[c].isna().sum()
    print(f"  Remaining NaN across all ec_*: {n_filled}")

    for c in CAT_COLS:
        dep[c] = pd.Categorical(dep[c], categories=train_cats[c])

    print("R_all_v24 3-seed mean predictions...")
    with open(os.path.join(MODELS, "lgbm_r_all_v24.features.txt")) as f:
        feat_all = f.read().splitlines()
    preds = []
    for s in SEEDS:
        b = lgb.Booster(model_file=os.path.join(MODELS, f"lgbm_r_all_v24_s{s}.txt"))
        preds.append(np.clip(b.predict(dep[feat_all]), 0, None))
        del b; gc.collect()
    r_all = np.mean(preds, axis=0)
    del preds; gc.collect()

    print("LIRF regime head v23...")
    ...
    with open(os.path.join(MODELS, "lirf_regime_v23.isotonic.pkl"), "rb") as f:
        iso = pickle.load(f)
    r_norm_lirf = np.clip(b_norm.predict(dep[feat_norm]), 0, None)
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v25.parquet"
    main(name)
