"""v26 submission = v24 pipeline but R_all trained without ec_* + opdi_*.

LIRF regime head (v23 p_fb + R_norm_LIRF) + Step A + ITY340 kept.
"""
...
def main(out_name="kind-mango_v26.parquet"):
    t0 = time.time()
    ...
    print("Features (v26 stack: no ec_* + no opdi_*; keep v22 stack for LIRF regime)...")
    ...
    dep = add_opdi(dep)         # kept for LIRF regime head
    dep = add_opdi_live(dep)    # kept for LIRF regime head
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v26.parquet"
    main(name)
