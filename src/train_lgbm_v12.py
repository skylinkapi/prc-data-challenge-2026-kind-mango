"""lgbm_v12 = v11 recipe + VRS aircraft-type metadata (manufacturer, engines, engine_type)."""
import glob
import os
import pickle
import time
...
from features_vrs import add_vrs, VRS_CAT_COLS, VRS_NUM_COLS
...
CAT_COLS = ["ADEP_mvt", "ADES_mvt", "RUNWAY_mvt", "STAND_mvt",
            ...
            "AIRCRAFT_OPERATOR_flt", "FLIGHT_RULE_mvt", "FLIGHT_TYPE_flt"] + VRS_CAT_COLS
...
def main():
    ...
    # Cap manufacturer to top-200 for LGBM's native categorical
    for c in CAT_COLS:
        # Take intersection with train values (avoid OTHER-only categories)
        if c in VRS_CAT_COLS:
            top = train[c].value_counts().head(200).index
            train[c] = train[c].where(train[c].isin(top), other="OTHER")
            test[c] = test[c].where(test[c].isin(top), other="OTHER")
    ...
    print(f"\nLGBM v12 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v11 = 294.18)")
    ...
    for c in VRS_CAT_COLS + VRS_NUM_COLS:
        ...
        print(f"  {c:25s} rank {r:3d}  gain {imp[c]:.2e}")

    out_dir = os.path.join(ROOT, "models")
    booster.save_model(os.path.join(out_dir, "lgbm_v12.txt"))
    with open(os.path.join(out_dir, "lgbm_v12.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v12.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v12.*")
