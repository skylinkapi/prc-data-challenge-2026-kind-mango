"""lgbm_v19 = v15 recipe + historical OPDI taxi climatology (2022-2024 baseline)."""
import glob
import os
import pickle
import time
...
from features_opdi_climate import add_opdi_climate, CLIM_NUM_COLS
...
def main():
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols + op_cols + TAXI_NUM_COLS + ADV_NUM_COLS + OSM_PATH_NUM_COLS + OPDI_NUM_COLS + OPDI_LIVE_NUM_COLS + CLIM_NUM_COLS
    print(f"Feature count: {len(feat)}  (v15 had 124)")

    print("Climate feature coverage:")
    for c in CLIM_NUM_COLS:
        print(f"  {c:25s}  {train[c].notna().mean()*100:5.1f}%")
    ...
    print(f"\nLGBM v19 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v15 = 293.09)")
    ...
    print("\nPer-airport RMSE (v19 vs v15):")
    print(tab.sort_values("rmse").to_string())

    print("\nClimatology feature ranks:")
    ...
    b.save_model(os.path.join(out_dir, "lgbm_v19.txt"))
    with open(os.path.join(out_dir, "lgbm_v19.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v19.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v19.*")
