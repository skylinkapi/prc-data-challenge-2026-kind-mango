"""v24 submission = 3-seed R_all mean + v23 LIRF regime + Step A + ITY340."""
...
def main(out_name="kind-mango_v24.parquet"):
    t0 = time.time()
    ...
    for i in np.where(ity_mask)[0]:
        ...
        print(f"  row sd={sd_r[i]:.0f}  pred={p_final[i]:.0f}")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v24.parquet"
    main(name)
