"""Apply extended 5-active-model ensemble (v7/v11/v15/v16/v17) to ranking.
Weights from ensemble_all.py NNLS (renormalized to sum=1 for stability).
"""
...
MODELS = os.path.join(ROOT, "submission", "..")   # not used
MODELS = os.path.join(ROOT, "models")

# Normalised NNLS weights (originally summed to 1.006)
_raw = {"v7": 0.1343, "v11": 0.0715, "v15": 0.4377, "v16": 0.0506, "v17": 0.3116}
_s = sum(_raw.values())
WEIGHTS = {k: v / _s for k, v in _raw.items()}
...
def main(out_name="kind-mango_v10.parquet"):
    t0 = time.time()
    print(f"Normalised weights: {WEIGHTS}")
    ...
    print("Loading training cats + SSL reference...")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v10.parquet"
    main(name)
