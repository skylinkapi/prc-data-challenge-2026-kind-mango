"""v23 = v22 pipeline + Step 2 fallback-rate encodings on p_fb.

Same as v22 but the LIRF regime head uses:
  - R_norm_LIRF (unchanged from v22)
  - p_fb_v23 (trained with 14 extra fallback-rate features)
  - isotonic calibrator from v23
"""
...
from train_lirf_regime_v23 import RATE_KEYS, RATE_FEATURES, K_SMOOTH
...
def add_rate_features_scoring(df, scoring_maps, base_rate):
    """Apply scoring-time (full-training) rate maps to a df."""
    key_defs = {
        "op": df["AIRCRAFT_OPERATOR_flt"].astype(str),
        "flt_prefix": df["FLIGHT_mvt"].astype(str).str[:3].fillna("UNK"),
        "stand": df["STAND_mvt"].astype(str),
        "aircraft_type": df["AIRCRAFT_TYPE_mvt"].astype(str),
        "op_stand": df["AIRCRAFT_OPERATOR_flt"].astype(str) + "|" +
                    df["STAND_mvt"].astype(str),
        "stand_first_char": df["STAND_mvt"].astype(str).str.slice(0, 1).fillna("UNK"),
        "destination": df["ADES_mvt"].astype(str),
    }
    for k, keys in key_defs.items():
        rmap = scoring_maps[k]["rate"]
        cmap = scoring_maps[k]["count"]
        vals = keys.values
        df[f"fbrate_{k}"] = np.array([rmap.get(v, base_rate) for v in vals], dtype=np.float32)
        df[f"fbcount_{k}"] = np.array([cmap.get(v, 0) for v in vals], dtype=np.float32)
    return df


def apply_stepA_v22(taxi_hat, sd, adep, fltid, table_data):
    """Refined Step A from v22."""
...
def main(out_name="kind-mango_v23.parquet"):
    t0 = time.time()
    ...
    # Apply v23 rate features to LIRF rows (all rows OK, but only LIRF classifier uses them)
    print("Applying fallback-rate features...")
    ...
    print("LIRF regime head v23 predictions...")
    ...
    print(f"LIRF regime head v23 applied to {lirf_mask.sum()} rows (mean p_fb {p_fb_cal[lirf_mask].mean():.3f})")
    ...
    print(f"Step A refined: {cell_A.sum()} rows")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v23.parquet"
    main(name)
