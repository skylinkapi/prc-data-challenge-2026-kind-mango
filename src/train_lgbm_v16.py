"""lgbm_v16 = v15 recipe + semi-supervised drift features from ranking rows."""
import glob
import os
import pickle
import time
...
from features_ssl import add_ssl, SSL_NUM_COLS
...
def main():
    ...
    # Training reference for SSL feature (all training DEP rows with sched_delay)
    training_ssl = dep[["ADEP_mvt", "AIRCRAFT_OPERATOR_flt", "sched_delay"]].copy()
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols + op_cols + TAXI_NUM_COLS + ADV_NUM_COLS + OSM_PATH_NUM_COLS + OPDI_NUM_COLS + OPDI_LIVE_NUM_COLS + SSL_NUM_COLS
    print(f"Feature count: {len(feat)}  (v15 had 124)")

    for c in SSL_NUM_COLS:
        cov = train[c].notna().mean() * 100
        print(f"  SSL {c:45s} train cov {cov:5.1f}%")
    ...
    print(f"\nLGBM v16 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v15 = 293.09)")
    ...
    for c in SSL_NUM_COLS:
        ...
        print(f"  {c:45s} rank {r:3d}  gain {imp[c]:.2e}")

    out_dir = os.path.join(ROOT, "models")
    b.save_model(os.path.join(out_dir, "lgbm_v16.txt"))
    with open(os.path.join(out_dir, "lgbm_v16.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v16.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v16.*")
