"""Fetch historical OPDI event chunks 2022-01 through 2024-12.

Same downloader as fetch_opdi_events.py but extended date range.
Stream-filters each chunk to keep only target airports + taxi event types.
Skips already-downloaded chunks (resume).

Expected: ~110 chunks × ~250 MB raw → ~1.5 GB filtered kept.
"""
import json
import os
import time
...
def parse_info_batch(info_series: pd.Series) -> pd.DataFrame:
    def _p(s):
        ...
        try:
            d = json.loads(s)
            return (d.get("osm_airport"), d.get("osn_flight_id"), d.get("osm_ref"))
        except Exception:
            return (None, None, None)
...
def main():
    start = date(2022, 1, 1)
    end   = date(2025, 1, 1)   # historical only; 2025+ already downloaded
    chunks = chunks_covering(start, end)
    print(f"Fetching {len(chunks)} historical OPDI chunks 2022-01 -> 2024-12", flush=True)
    for a, b in chunks:
