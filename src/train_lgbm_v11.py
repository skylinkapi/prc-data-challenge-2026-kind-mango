"""lgbm_v11 = v10 recipe + OPDI rolling event counts."""
import glob
import os
import pickle
import time
...
def main():
    ...
    print("Adding features (all layers + OPDI)...")
    t = time.time()
    # keep only needed columns from movements to reduce RAM
    ...
    print(f"Feature count: {len(feat)}  (v10 had 106)")

    print("\nOPDI feature coverage:")
    for c in OPDI_NUM_COLS:
        cov = train[c].notna().mean() * 100
        print(f"  {c:40s} coverage {cov:5.1f}%")
    ...
    print(f"\nLGBM v11 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v10 = 294.40)")
    ...
    v10 = pd.Series({"LEMD":204.58,"LSZH":227.69,"LEBL":239.81,"EDDF":243.97,"EDDM":245.14,
                     "EHAM":249.63,"LTFM":293.21,"LFPG":329.94,"EGLL":359.98,"LIRF":462.04})
    tab["v10"] = v10
    tab["delta"] = (tab["rmse"] - tab["v10"]).round(2)
    print("\nPer-airport RMSE (v11 vs v10):")
    print(tab.sort_values("rmse").to_string())

    print("\nOPDI feature ranks (of 115 features):")
    ...
    for c in OPDI_NUM_COLS:
        ...
        print(f"  {c:40s} rank {r:3d}  gain {imp[c]:.2e}")

    out_dir = os.path.join(ROOT, "models")
    booster.save_model(os.path.join(out_dir, "lgbm_v11.txt"))
    with open(os.path.join(out_dir, "lgbm_v11.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v11.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v11.*")
