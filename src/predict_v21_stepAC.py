"""Submission: v21 + Step A LIRF rule + Step C fallback detector soft mix.

Prediction pipeline for each ranking row:
  1. taxi_hat = v21 booster prediction
  2. If ADEP=LIRF, FLIGHT_ID null, sd>14400: apply Step A band rule
  3. Else if ADEP in {LIRF, EGLL} and sd>3600: p_final = p_fb*sd + (1-p_fb)*taxi_hat
     (soft mixture; unconditional on p_fb per hold-out sweep)
"""
...
def main(out_name="kind-mango_v17.parquet"):
    t0 = time.time()
    ...
    print(f"Step A applied to {cell_A.sum()} rows")

    # Step C: LIRF + EGLL soft-mix outside Step A cell
    p_final = p_A.copy()
    for apt in ["LIRF", "EGLL"]:
        ...
        sub_mask = (adep_r == apt) & (sd_r > 3600) & ~cell_A & ~np.isnan(sd_r)
        if sub_mask.sum() == 0: continue
        sub = dep.loc[sub_mask].copy()
        for c in DET_CAT:
            if c in sub.columns:
                sub[c] = sub[c].astype("category")
        p_fb = det.predict(sub[DET_CAT + DET_NUM])
        # soft mix, unconditional threshold (per hold-out sweep the best)
        p_final[sub_mask] = p_fb * sd_r[sub_mask] + (1 - p_fb) * p_A[sub_mask]
        print(f"  Step C {apt}: {sub_mask.sum()} rows,  mean p_fb {p_fb.mean():.3f},  "
              f"max shift {np.max(np.abs(p_final[sub_mask]-p_A[sub_mask])):.0f}")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v17.parquet"
    main(name)
