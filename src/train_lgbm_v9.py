"""lgbm_v9 = v6 recipe + 5 new physically-motivated features.

v6 features (97 base + flt_null = 98) plus:
...
Total 103 features. Same Optuna params as v5. Same BLOCK==SCHED cleaning.
"""
import glob
import os
import pickle
...
def main():
    ...
    t = time.time(); print("Adding features (weather + congestion + EC + operator + distance + advanced)...")
    dep = add_weather(dep)
    ...
    all_mvt = m[["ADEP_mvt", "PHASE_mvt", "mvt_ts", "sched_ts", "RUNWAY_mvt"]].copy()
    dep = add_advanced(dep, all_mvt, daily_ec=ec_daily)
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols + op_cols + TAXI_NUM_COLS + ADV_NUM_COLS
    print(f"Feature count: {len(feat)}  (v6 had 98)")
    ...
    print(f"\nLGBM v9 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v6 = 295.56, v5 = 295.49)")
    ...
    v6 = pd.Series({"LEMD":206.59,"LSZH":228.55,"LEBL":241.50,"EDDF":245.92,"EDDM":246.25,
                    "EHAM":249.63,"LTFM":294.03,"LFPG":330.63,"EGLL":362.36,"LIRF":462.58})
    tab["v6"] = v6
    tab["delta"] = (tab["rmse"] - tab["v6"]).round(2)
    print("\nPer-airport RMSE (v9 vs v6):")
    print(tab.sort_values("rmse").to_string())

    print("\nAdvanced-feature gain ranks (of top 30 features by gain):")
    imp = pd.Series(booster.feature_importance(importance_type="gain"),
                    index=feat).sort_values(ascending=False)
    top30 = list(imp.head(30).index)
    for c in ADV_NUM_COLS:
        r = list(imp.index).index(c) + 1
        marker = " ***top30***" if c in top30 else ""
        print(f"  {c:40s} rank {r:3d}  gain {imp[c]:.2e}{marker}")

    out_dir = os.path.join(ROOT, "models")
    booster.save_model(os.path.join(out_dir, "lgbm_v9.txt"))
    with open(os.path.join(out_dir, "lgbm_v9.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v9.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v9.*")
