"""Train R_all_v21 = R_all_v20 recipe with Step 1 defect fixes + Step 2 linear_tree
+ Step 7 temperature features.

Changes vs v20:
  - features_opdi_live.py: 600 s lag (fix 3.1)
  - features_turnaround.py: read BLOCK_TIME_UTC_mvt directly (fix 3.5)
  - features_weather.py: add tmpc, dwpc, dewpt_spread, ice_accretion_1hr, deicing_gate
  - Add signed-log copies of sd, mvt_eobt1, mvt_iobt, eobt1_sched, eobt1_iobt
  - LightGBM linear_tree=True, linear_lambda=1.0, num_leaves=220
"""
...
# Signed-log columns for linear_tree
SIGNED_LOG_BASE = ["sched_delay", "mvt_eobt1", "mvt_iobt", "eobt1_sched", "eobt1_iobt"]
SIGNED_LOG_COLS = [f"{c}_slog" for c in SIGNED_LOG_BASE]
TEMPERATURE_COLS = ["tmpc", "dwpc", "dewpt_spread", "ice_accretion_1hr", "deicing_gate"]
...
def main():
    ...
    # Signed-log copies for linear_tree (Step 2)
    ...
    print("Features (v21 stack: v20 + new temperature + fixes)...")
    t = time.time()
    dep = add_weather(dep)
    ...
    print(f"Feature count: {len(feat)}  (v20 had 143)")
    ...
    # Step 2: linear_tree
    lin_features = list(set(SIGNED_LOG_COLS + ["sched_delay", "mvt_eobt1", "mvt_iobt",
                                                "eobt1_sched", "eobt1_iobt"]) & set(feat))
    lin_features = [c for c in lin_features if train[c].dtype != "category"]
    print(f"linear_tree features: {lin_features}")
    ...
    # Step 2 params: linear_tree true, num_leaves 220
    params = {**BEST_PARAMS,
    ...
    print("\nTraining R_all v21 with linear_tree...")
    t0 = time.time()
    ...
    print(f"\n=== R_all v21 Jan+Jul evaluation ===")
    print(f"FULL   RMSE: {rmse(y_te, p_te):.2f}  (v20 was 456.40)")
    clean = (y_te >= 30) & (y_te <= 7200)
    print(f"CLEAN  RMSE: {rmse(y_te[clean], p_te[clean]):.2f}  (v20 was 280.17)  n={clean.sum():,}")

    print("\nPer-airport clean RMSE:")
    v20_clean = {"LIRF":571.56,"EGLL":283.53,"LFPG":286.36,"LTFM":260.44,"LEBL":229.38,
                 "EDDF":223.32,"LSZH":213.00,"EHAM":214.87,"EDDM":211.34,"LEMD":190.22}
    apt_te = test["ADEP_mvt"].astype(str).values
    for apt in sorted(v20_clean, key=lambda k: -v20_clean[k]):
        mask = (apt_te == apt) & clean
        if mask.sum() > 0:
            r = rmse(y_te[mask], p_te[mask])
            print(f"  {apt}: v21 {r:>6.2f}   v20 {v20_clean[apt]:>6.2f}   delta {r-v20_clean[apt]:+.2f}")

    imp = pd.Series(b.feature_importance(importance_type="gain"),
                    index=feat).sort_values(ascending=False)
    print("\nNew feature ranks:")
    for c in SIGNED_LOG_COLS + TEMPERATURE_COLS:
        if c in imp.index:
            r = list(imp.index).index(c) + 1
            print(f"  {c:22s} rank {r:>3d}/{len(feat)}  gain {imp[c]:.2e}")

    out_dir = os.path.join(ROOT, "models")
    b.save_model(os.path.join(out_dir, "lgbm_r_all_v21.txt"))
    with open(os.path.join(out_dir, "lgbm_r_all_v21.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_r_all_v21.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_r_all_v21.*   total wall {time.time()-t_all:.1f}s")
