"""lgbm_v18 = v15 recipe + openap physical aircraft features (MTOW, wingspan, engines, pax)."""
import glob
import os
import pickle
import time
...
from features_openap import add_openap, OPENAP_NUM_COLS
...
def main():
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols + op_cols + TAXI_NUM_COLS + ADV_NUM_COLS + OSM_PATH_NUM_COLS + OPDI_NUM_COLS + OPDI_LIVE_NUM_COLS + OPENAP_NUM_COLS
    print(f"Feature count: {len(feat)}  (v15 had 124)")

    print("openap coverage:")
    for c in OPENAP_NUM_COLS:
        print(f"  {c:20s}  {train[c].notna().mean()*100:5.1f}%")
    ...
    print(f"\nLGBM v18 hold-out RMSE: {rmse(y_te, p_te):.2f}s   (v15 = 293.09)")
    ...
    print("\nPer-airport RMSE (v18 vs v15):")
    print(tab.sort_values("rmse").to_string())

    print("\nopenap feature ranks:")
    imp = pd.Series(b.feature_importance(importance_type="gain"),
                    index=feat).sort_values(ascending=False)
    for c in OPENAP_NUM_COLS:
        r = list(imp.index).index(c) + 1
        print(f"  {c:20s} rank {r:3d}  gain {imp[c]:.2e}")

    out_dir = os.path.join(ROOT, "models")
    b.save_model(os.path.join(out_dir, "lgbm_v18.txt"))
    with open(os.path.join(out_dir, "lgbm_v18.features.txt"), "w") as f:
        f.write("\n".join(feat))
    with open(os.path.join(out_dir, "lgbm_v18.encoders.pkl"), "wb") as f:
        pickle.dump(encoders, f)
    print(f"\nSaved -> {out_dir}/lgbm_v18.*")
