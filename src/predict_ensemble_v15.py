"""Apply 5-model ensemble (v7/v11/v10/v15) to ranking. Weights from ensemble_v15.py."""
import os
import pickle
import sys
import time
...
WEIGHTS = {"v7": 0.15, "v10": 0.05, "v11": 0.15, "v15": 0.65}
...
def main(out_name: str = "kind-mango_v7.parquet"):
    t0 = time.time()
    print("Loading ranking...")
    rank = pd.read_parquet(RANK)
    tmpl = pd.read_parquet(SUB_TMPL)
    print(f"  ranking {len(rank):,}   template {len(tmpl):,}")
    ...
    with open(os.path.join(MODELS_DIR, "lgbm_v15.encoders.pkl"), "rb") as f:
    ...
    assert set(out["MVT_ID_mvt"]) == set(tmpl["MVT_ID_mvt"])
    ...
    if n_nan:
        ...
        print(f"[warn] {n_nan} NaN, filled with median {med:.0f}s")
    out["TAXITIME_SEC_mvt"] = out["TAXITIME_SEC_mvt"].clip(60, 7200)
    print(f"[ok] rows={len(out):,}")

    path = os.path.join(ROOT, "submission", out_name)
    out.to_parquet(path)
    print(f"\nSaved -> {path}   ({os.path.getsize(path)/1024:.1f} KB)  ({time.time()-t0:.1f}s)")


if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v7.parquet"
    main(name)
