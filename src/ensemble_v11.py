"""6-model ensemble grid search over v5, v6, v7, v9, v10, v11.

v11 uses OPDI features on top of v10's set. Its errors should be modestly
decorrelated with v10 (OPDI vs OSM-path emphasis), enabling small further
RMSE drop from blending.

Coarser grid than 5-model version (step 0.05 across 6 dims = ~53k combos).
"""
import glob
import os
import pickle
...
def main():
    ...
    print("6-simplex grid step 0.05...")
    t0 = time.time()
    ...
    tab = pd.DataFrame({"apt": apt, "y": y, "p": ens, "p_v11": preds["v11"]}) \
        .groupby("apt").apply(
            lambda g: pd.Series({"n": len(g),
                                 "rmse_ens": rmse(g["y"], g["p"]),
                                 "rmse_v11": rmse(g["y"], g["p_v11"])}),
            include_groups=False).round(2)
    tab["delta"] = (tab["rmse_ens"] - tab["rmse_v11"]).round(2)
    print("\nPer-airport (ensemble vs v11 solo):")
    print(tab.sort_values("rmse_ens").to_string())

    with open(os.path.join(MODELS_DIR, "ensemble_weights_v11.txt"), "w") as f:
        for t, wt in zip(tags, w):
            f.write(f"{t}: {wt}\n")
    print(f"\nSaved -> {MODELS_DIR}/ensemble_weights_v11.txt")
