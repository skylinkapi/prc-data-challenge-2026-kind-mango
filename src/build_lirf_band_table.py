"""Step A: build the LIRF null-flight-record band lookup table.

For each `sd` band, compute on the FULL 12 months of 2025:
  P(fb)      = share of rows where |y - sd| < 60
  P(24h)     = share of rows where y > 80,000
  mean_norm  = mean y of the 'normal' rows (not fb, not 24h)
  mean_24h_extra = mean of (y - 86,400) for the 24h rows
  n          = row count

Applied only when ADEP_mvt == 'LIRF' AND FLIGHT_ID_mvt is null AND sd > 14,400.
Saves a JSON with bands + stats. Also reports the hold-out check.
"""
...
GATE_SD = 14400  # apply rule only above this
...
def main():
    ...
    print(f"LIRF DEP rows: {len(m):,}   null flight: {m['null_flt'].sum():,}")
    ...
    for band_id in [f"{lo}_{hi}" for lo, hi in BANDS]:
        ...
        table[band_id] = {
            "n": int(len(sub)),
            "p_fb": p_fb, "p_24h": p_24, "p_norm": p_norm,
            "mean_norm": mean_norm, "mean_24h_extra": mean_24h_extra,
        }
        print(f"  {band_id:15s} n={len(sub):>4}  P_fb={p_fb:.3f}  P_24h={p_24:.3f}  "
              f"P_norm={p_norm:.3f}  mean_norm={mean_norm if not np.isnan(mean_norm) else 'nan':>7}  "
              f"mean_24h_extra={mean_24h_extra if not np.isnan(mean_24h_extra) else 'nan':>7}")

    out_path = os.path.join(MODELS, "lirf_band_table.json")
