"""Submission: v21 predictions + Step A LIRF band rule."""
...
def main(out_name="kind-mango_v16.parquet"):
    t0 = time.time()
    ...
    p_final, cell = apply_rule(taxi_hat, sd_r, adep_r, fltid_r, table_data)
    print(f"Rule applied to {cell.sum()} rows (expected ~43)")
    print(f"Cell prediction stats: min {p_final[cell].min():.0f}  max {p_final[cell].max():.0f}  mean {p_final[cell].mean():.0f}")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v16.parquet"
    main(name)
