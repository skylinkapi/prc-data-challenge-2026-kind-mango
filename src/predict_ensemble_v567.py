"""Apply v5+v6+v7 ensemble to ranking.parquet.

Blend weights from grid search on 2025 hold-out:
  v5: 0.30, v6: 0.32, v7: 0.38.
"""
import os
import pickle
import sys
...
WEIGHTS = {"v5": 0.30, "v6": 0.32, "v7": 0.38}
...
def main(out_name: str = "kind-mango_v2.parquet"):
    t0 = time.time()
    ...
    print(f"[ok] rows={len(out):,}, id set matches, dtypes {list(out.dtypes)}")
...
if __name__ == "__main__":
    name = sys.argv[1] if len(sys.argv) > 1 else "kind-mango_v2.parquet"
    main(name)
