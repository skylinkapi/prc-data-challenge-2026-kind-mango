"""LightGBM v2: v1 features + Eurocontrol daily-per-airport features.

Target: reduce LIRF RMSE (482 s in v1) by feeding the model daily ATFM and
pre-departure delay signals that correlate with tail flights.
"""
import glob
import os
import time
...
def main():
    ...
    t0 = time.time()
    print("Building DEP + weather + congestion + eurocontrol...")
    ...
    print(f"  {len(dep):,} rows in {time.time()-t0:.1f}s")
    ...
    feat = CAT_COLS + NUM_COLS_V1 + ec_cols
    print(f"Feature count: {len(feat)}  (v1 had {len(CAT_COLS)+len(NUM_COLS_V1)})")
    ...
    print(f"\nLGBM v2 overall RMSE: {rmse(y_te, p_te):.1f}s   (v1 = 302.2s)")
    ...
    v1 = pd.Series({"LEMD":209.4,"LSZH":230.6,"LEBL":243.6,"EDDF":250.1,"EDDM":250.3,
                    "EHAM":252.3,"LTFM":302.8,"LFPG":337.4,"EGLL":368.6,"LIRF":482.2})
    tab["v1"] = v1
    tab["delta"] = (tab["rmse"] - tab["v1"]).round(1)
    print("\nPer-airport RMSE (v2 vs v1):")
    ...
    booster.save_model(os.path.join(out_dir, "lgbm_v2.txt"))
    with open(os.path.join(out_dir, "lgbm_v2.features.txt"), "w") as f:
        f.write("\n".join(feat))
    print(f"\nSaved -> {out_dir}/lgbm_v2.txt")
