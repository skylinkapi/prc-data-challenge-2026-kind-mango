"""L3.c: paired test of terminal (airport × stand_prefix) tempo on v44.

Adds the 3 terminal 30-min columns to the v44 feature set (116 cols) and
trains the v26 recipe twice: control (v44) and v45term (119 cols). Writes a
...
TERM30 = os.path.join(MODELS, "tempo_term30_train.parquet")
EXTRA_COLS = ["nb_eobt_med_term30", "nb_eobt_mean_term30", "nb_eobt_cnt_term30"]
...
def load_frame() -> tuple[pd.DataFrame, list[str]]:
    ...
    dep = dep.merge(pd.read_parquet(TERM30), on="MVT_ID_mvt", how="left")
    cov = dep["nb_eobt_med_term30"].notna().mean()
    log.info("frame %d rows, term30 coverage %.3f", len(dep), cov)
    return dep, feat
...
def main() -> None:
    ...
    for tag, cols in (("control_v44", control_cols), ("v45term", v45_cols)):
    ...
    delta_class = {k: report["v45term"]["class_mse"][k] - report["control_v44"]["class_mse"][k]
    ...
    for k in delta_class:
        ...
        new = report["v45term"]["class_mse"][k] - report["v45term"]["per_airport"]["LIRF"][k]
        served[k] = new - ctl
    report["delta_v45term_minus_v44"] = {
        "full_rmse": report["v45term"]["full_rmse"] - report["control_v44"]["full_rmse"],
        "clean_rmse": report["v45term"]["clean_rmse"] - report["control_v44"]["clean_rmse"],
        "class_mse": delta_class,
    }
    report["delta_served_v45term_minus_v44"] = served
    ...
    with open(os.path.join(MODELS, "lgbm_r_all_v45term.holdout.json"), "w") as f:
