...
Status as of 2026-09-26. v71 (MF2 runway-queue columns) landed -0.015 s, inside noise, and is not accepted. v68 (CatBoost in `R_norm`) landed +0.03 s and is rejected. Team best **281.87 s (v67)**: v65 plus a CatBoost member blended 50/50 into the base outside LIRF (MB7, WINNING_PLAN L3), -2.87 s. Status of 2026-09-23 follows. Team best then **284.74 s (v65)**: v64 plus the `plan_nm_taxi` base column (MF5, WINNING_PLAN L9), -1.22 s. Before that, v64 285.95 s: v63 plus an out-of-fold isotonic map on the LIRF gate (L3, WINNING_PLAN L10), -1.14 s. Before that, v63 287.09 s: v57 with the Step A normal term read from `R_norm` (T1, WINNING_PLAN L12), -0.24 s. v61 (WINNING_PLAN L4, v45 parameters on the v57 recipe) rejected +0.99 s. L13 (MS3 on the v57 base) moved 0 rows and was not uploaded. L10 landed as v64 and L9 as v65. L11 (weather at EOBT_1) as v66 was rejected +0.33 s; v65 stays best. The Discord tail rules were priced on the 2025 hold-out and closed. `docs/MODEL_ANALYSIS.md` Progress section tracks the merged to-do list. Status of 2026-09-19 end-of-day follows. Team best **287.33 s** (v57, live, -11.98 s vs v41). MB3 base filter (v51 -5.99 s) plus the MB8 rollout: R_norm 12-month refit (v55 -0.42 s), p_fb gate 12-month refit (v56 -0.33 s) and plan_taxi_res route medians 12-month refit (v57 -0.17 s). Winning theme: broader training exposure + label-free defensive gates. v58 (MB8 for operator encoders) rejected +1.29 s (encoder leak amplified). v59 (MF4 arrival drift features) rejected +0.37 s (drift signal did not transfer).
