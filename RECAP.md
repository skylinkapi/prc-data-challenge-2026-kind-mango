...
| tag | RMSE   | notes                                                |
|-----|--------|------------------------------------------------------|
| v48 | pending | Fifteenth-pass Phase 1: MS1 per-airport upper bounds + MS2 clean floor applied post-hoc to the v47 parquet (`src_v3/build_v48.py`, `src_v3/postprocess.py`, `src_v3/support.py`). MS1 caps at the 2025 tail-class max plus a 600 s margin at EDDF/EDDM/LEBL/LEMD (no long tail), and at the clean max plus margin at EGLL/EHAM/LFPG/LSZH/LTFM unless the row has flt_null=0 and mvt_eobt1 > 5400. MS2 floors at the 2025 clean 0.1 %-quantile per airport. MS1 moved 5 non-LIRF rows (EDDM 21,465 -> 7,499 the biggest -13,966 s; EHAM 11,103 -> 7,311; two LEBL rows to 4,204; LEMD 4,376 -> 4,198). MS2 moved 104 rows (small pushes). LIRF head output untouched by MS1; MS2 raised 14 LIRF rows by tiny amounts. Zero training cost. MP7 gate passed. Uploaded 2026-09-17, score pending. |
