...
| tag | RMSE   | notes                                                |
|-----|--------|------------------------------------------------------|
| v60 | pending | Fifteenth-pass MD4: leave-one-month-out operator encoders (correct fix for the v58 encoder leak). `src_v3/train_v60_base.py` computes 12 fold-encoders per key (each fit on 11 months excluding the row's own month) and produces OOF-encoded openc_* columns for training rows. The full-year encoder is saved for serving on 2026 rows. Retrained the v57 base with these OOF columns; MB3 filter, retuned params, 3 seeds, same iter scaling. Non-LIRF mean shifts vs v57: LTFM +11.73 s (biggest, mean|d| 42.94), EDDM +4.55, EDDF +3.37, EHAM -2.36; LIRF unchanged. Direction is opposite v58 (leak removal, not amplification). MP7 gate passed. Uploaded 2026-09-20. |
