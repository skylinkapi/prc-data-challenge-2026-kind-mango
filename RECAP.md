...
Status as of 2026-09-26. Reproducibility pass: `requirements.txt` pins the libraries; `REPRODUCE.md` covers v67 in two tiers (serve from artefacts, retrain every served artefact); `src_v3/test_v67_parity.py` rebuilds v67 bit for bit against a recorded hash; the H1 id side file is now written by `tune_lgbm_v36.py build` and a scratch rebuild matched the historical cache and ids exactly; the v56 gate reads its round count from config, not from the v23 booster; the five feature lists v67 reads are tracked. Known gaps: no release bundle of the 936 MB served artefacts, no METAR fetch script, no end-to-end Tier-2 run on a clean clone. v71 (MF2 runway-queue columns) landed -0.015 s, inside noise, and is not accepted. v68 (CatBoost in `R_norm`) landed +0.03 s and is rejected. Team best **281.87 s (v67)**: v65 plus a CatBoost member blended 50/50 into the base outside LIRF (MB7, WINNING_PLAN L3), -2.87 s. Status of 2026-09-23 follows. Team best then **284.74 s (v65)**: v64 plus the `plan_nm_taxi` base column (MF5, WINNING_PLAN L9), -1.22 s. Before that, v64 285.95 s: v63 plus an out-of-fold isotonic map on the LIRF gate (L3, WINNING_PLAN L10), -1.14 s. Before that, v63 287.09 s: v57 with the Step A normal term read from `R_norm` (T1, WINNING_PLAN L12), -0.24 s. v61 (WINNING_PLAN L4, v45 parameters on the v57 recipe) rejected +0.99 s. L13 (MS3 on the v57 base) moved 0 rows and was not uploaded. L10 landed as v64 and L9 as v65. L11 (weather at EOBT_1) as v66 was rejected +0.33 s; v65 stays best. The Discord tail rules were priced on the 2025 hold-out and closed. `docs/MODEL_ANALYSIS.md` Progress section tracks the merged to-do list. Status of 2026-09-19 end-of-day follows. Team best **287.33 s** (v57, live, -11.98 s vs v41). MB3 base filter (v51 -5.99 s) plus the MB8 rollout: R_norm 12-month refit (v55 -0.42 s), p_fb gate 12-month refit (v56 -0.33 s) and plan_taxi_res route medians 12-month refit (v57 -0.17 s). Winning theme: broader training exposure + label-free defensive gates. v58 (MB8 for operator encoders) rejected +1.29 s (encoder leak amplified). v59 (MF4 arrival drift features) rejected +0.37 s (drift signal did not transfer).
...
| tag | RMSE   | notes                                                |
|-----|--------|------------------------------------------------------|
| v72 | not built | MH2 from MODEL_ANALYSIS 5.4, fallback heads outside LIRF. `src_v3/measure_mh2.py`: a LightGBM classifier for `abs(y - sd) < 60` on the v65 columns and MB3 rows of the v46 months (early stop at 232 rounds on months 11 and 12, isotonic map on the months 11 and 12 scores) scores Jan+Jul 2025 (fallback rate 7.5 %). Against the served-style base 0.5 v46 + 0.5 CatBoost A (saved hold-out predictions), `base + alpha p (sd - base)` costs +292 MSE at alpha 1 and +72 at alpha 0.5, worse at every one of the 9 airports. Outside LIRF a row with y near sd is mostly an on-time pushback, which the base, trained on every row, already serves. Closed; no upload. 2026-09-26. |
| v71 | 281.85 | -0.015 s vs v67 (not accepted: inside noise, bar -0.3 s). MF2 from MODEL_ANALYSIS 5.5: `src_v3/build_runway_queue_v71.py` writes seven backward-window columns per departure from every movement at the airport (MD8 pool): same-runway departures and heavy (H/J) departures in the last 5 and 10 min, the heavy share, departures in the last 10 min to a destination within 30 degrees of the row's bearing (OurAirports coordinates, 99.7 % coverage), and minutes since the set of runways used in the last 30 min changed. A first build read microsecond timestamps as nanoseconds, so every window was 1,000 times too long; the unit-free conversion fixed it before any fit. Values match between 2025 and 2026 (about 4 departures per runway per 10 min, heavy share 0.21, set age median about 2 h). Hold-out (`src_v3/measure_mf2.py`, one LightGBM member per arm, v43 parameters, v46 months): early-stopped arms stopped at 438 and 1,959 rounds and gave -1,861 MSE (`mf2_earlystop.holdout.json`), inflated by the short control; at equal fixed rounds the price is -559 / -524 / -586 / -596 MSE at 500 / 1,000 / 1,500 / 2,000 rounds, EDDF about -300. v71 = LightGBM base retrained with the MF2 columns (`src_v3/train_v71_base.py`, 125 columns) blended 0.5 with the v67 CatBoost, which lacks them (`predict_v67 --base-model lgbm_r_all_v71 --extra ...`). Label-free check vs v67: LIRF unchanged; shifts within +/-0.5 s; median |d| 5 s, max 1,136 s. Live -0.015 s (-8 MSE). OSM runway crossings and the remote-runway flag were not built. Uploaded 2026-09-25 (UTC). |
| v70 | not built | Option B, a second base CatBoost. `python -m src_v3.measure_catboost_pair` refits the v67 member A (5,000-round cap) and fits variant B (`CATBOOST_PARAMS_B`: depth 10, seed 43, `rsm` 0.5; early stop at 4,810 rounds) on the v46 months, saves both hold-out predictions (`models/v3/catboost_pair_holdout.parquet`), and prices against v46: 0.5 v46 + 0.5 A -2,079 (reproduces v67), 0.5 v46 + 0.25 A + 0.25 B -2,150, 0.5 v46 + 0.5 B -1,946 MSE. The pair adds 71 MSE, under the 150 MSE bar set before the run, so the full-year fit and the upload did not run. v67 stays best. 2026-09-25. |
| v69 | not built | Option A, converged base CatBoost. `python -m src_v3.measure_catboost_blend --tag v69 --max-rounds 15000`: early stop at 6,480 rounds on months 11 and 12. Hold-out against v46: CatBoost alone -1,474 MSE; blend w 0.3 -1,647, w 0.5 -2,171. The first 5,000 trees of the same fit give -1,584 and -2,079, the v67 numbers. The converged model adds 92 MSE at w 0.5, under the 150 MSE bar set before the run (about 0.3 s live at the v67 transfer rate), so the 5 h full-year fit and the upload did not run. v67 stays best. `catboost_base`, `measure_catboost_blend` and `predict_v67` now take a version tag. 2026-09-25. |
