...
| tag | RMSE   | notes                                                |
|-----|--------|------------------------------------------------------|
| v52 | pending | Fifteenth-pass Phase 1: MB3+MB4 base. MB3 filter kept from v51 (LIRF and y > 80,000 excluded), plus MB4 calendar features: doy_sin/doy_cos (cyclic day-of-year), hour_local (DST-aware per airport timezone) and is_public_hol. Dropped the numeric `month` column per §5.3 MB4. 1,923,953 rows, 120 features (was 117 in v47/v51), same retuned params. Same iter scaling as v51 (767, 2265, 1278). MB4 features coverage 0.9999 (some rows have NaN mvt_ts). Post-processing (MS1+MS2) moved only 86 rows with max 304 s. Non-LIRF shifts vs v51: EGLL -4.25, LTFM -4.93, LEMD -3.12, EDDF -1.89 (biggest), others under 1 s. LIRF unchanged. Uploaded 2026-09-18. |
| v51 | pending | Fifteenth-pass Phase 1: MB3 base retrained on served rows only. `src_v3/train_v51_base.py` filters out LIRF rows and rows with y > 80,000, then trains the retuned v47 recipe on the 1,923,953 remaining rows across all 12 months (dropped 160,706 = 7.7 %). Round counts scaled from the v46 paired iters by the served-row growth ratio (1.259 vs 1.136 for v47's 1/0.88). 3 seeds: 767, 2265, 1278 rounds. `predict_v51.py` serves the new base with the v41 LIRF head unchanged plus MS1 + MS2. Sanity: post-MS moved only 85 rows with max |diff| 304 s (v48 had 109 rows, max 13,966 s) - the MB3 base already avoids the extreme predictions natively because its linear leaves are no longer shaped by LIRF's 16.6 % fallback rate and the 12 24-h rows. Non-LIRF shifts vs v48: EHAM +6.25 (largest), all others under 1 s absolute. Uploaded 2026-09-18. |
