...
Status as of 2026-09-17. Team best **294.24 s** (v48, live, -5.07 s vs v41).
...
| tag | RMSE   | notes                                                |
|-----|--------|------------------------------------------------------|
| v49 | pending | Fifteenth-pass Phase 1 second ship: MD2 + MH1 rebuild of the LIRF head. `src_v3/train_lirf_head_v49.py` retrained the R_norm_LIRF (5 seeds) and the p_fb gate + isotonic without the 38 `ec_*` + 9 `opdi_*` + 9 `opdi_live_*` columns (56 drifted columns per D1, coverage 100 % in 2025 to 0 % in July 2026). Rate features (fbrate/fbcount) kept. R_norm 110 features (was 166); p_fb 111 features (was 167). Paired LIRF-only hold-out (26,528 rows): v41 head FULL 887.42 CLEAN 463.55; v49 head FULL 869.28 CLEAN 460.69. LIRF MSE contribution to the 344,341-row scale: v41 60,619 -> v49 58,214, delta -2,405 MSE paired. p_fb stop AUC 0.861 (v23 was 0.858), log-loss 0.366 (v23 0.369). Base and MS1/MS2 unchanged from v48. Non-LIRF rows unchanged; LIRF mean shift -13.94 s across 26,805 rows. MP7 gate passed. Uploaded 2026-09-17, score pending. |
